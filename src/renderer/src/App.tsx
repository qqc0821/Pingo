...
import { projectTaskEvent, type TaskProjectionState } from "./taskEventProjection.js"
...
function readPromptHistory(): PetPromptItem[] {
  try {
    ...
    return parsed.filter(isPersistablePrompt).slice(-MAX_PROMPT_CARDS)
  } catch {
    return []
  }
}
...
export function App(): ReactElement {
  ...
  const taskProjection = useRef<TaskProjectionState>({
    response: "",
    operationResult: "",
    request: "",
  })
  ...
  useEffect(() => {
    if (prompts.length <= MAX_PROMPT_CARDS) return
    setPrompts((current) => {
      const active = current.filter((item) => item.sticky).slice(-MAX_PROMPT_CARDS)
      const room = MAX_PROMPT_CARDS - active.length
      const recent = room > 0 ? current.filter((item) => !item.sticky).slice(-room) : []
      return [...recent, ...active].sort((a, b) => a.createdAt - b.createdAt)
    })
  }, [prompts])
  ...
  useEffect(() => {
    ...
    const persistable = [...historyById.values()]
      .sort((left, right) => left.createdAt - right.createdAt)
      .slice(-MAX_PROMPT_CARDS)
    try {
  ...
  const handleTaskEvent = useCallback(
    (event: ChatStreamEvent) => {
      if (!taskIsActive.current) return
      const projected = projectTaskEvent(taskProjection.current, event)
      taskProjection.current = projected.state
      if (projected.patch) updateTaskCard(projected.patch)
      if (projected.sending !== undefined) setIsSending(projected.sending)
      ...
      if (projected.petState) {
        showPetState(projected.petState, projected.terminal ? HAPPY_STATE_DURATION_MS : undefined)
      }
    },
  ...
  useEffect(() => {
    ...
    void api.task
      .getSnapshot()
      .then((snapshot) => {
        if (!mounted || taskIsActive.current) return
        if (!snapshot) {
          void api.task
            .getRecovery()
            .then((recovery) => {
              if (!mounted || !recovery || taskIsActive.current) return
              const now = Date.now()
              const content = `上次处理“${recovery.interruptedInput.slice(0, 100)}”时应用已退出。任务已中断，未自动重试。`
              setPrompts((current) =>
                upsertPromptItem(current, {
                  id: createPromptId("interrupted"),
                  dedupKey: "session-interrupted",
                  kind: "result",
                  tone: "warning",
                  label: "已中断",
                  content,
                  summary: summarizePromptContent(content),
                  expandable: true,
                  createdAt: now,
                  updatedAt: now,
                }),
        ...
        taskProjection.current = { response: "", operationResult: "", request: snapshot.content }
  ...
  const submitMessage = useCallback(
    (event?: FormEvent<HTMLFormElement>) => {
      ...
      activeTaskId.current = null
      taskKey.current = createPromptId("task")
      taskProjection.current = { response: "", operationResult: "", request: content }
      setExpandedCardId(null)
      setActivePromptId(null)
      setDraft("")
