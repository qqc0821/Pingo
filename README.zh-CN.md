...
> **MVP 状态：** Pingo 是可运行的 macOS 早期版本。上方分镜使用 Pingo 当前视觉素材预览产品工作流；有限的对话上下文会保存在本地，重启后可以继续。中断的任务会标明状态，不会自动重新执行。
...
- 对话上下文另存在有版本与大小限制的 `0600` 会话文件中；“新对话”会清除当前会话，中断运行不会自动续跑。
