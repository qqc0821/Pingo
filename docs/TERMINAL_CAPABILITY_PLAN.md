...
- 用户首次要求 Pingo 操作电脑时，必须先授予明确、可见、可撤销的能力权限；未授权时只允许生成操作计划，不允许执行。
- 文件写入、移动、删除以及任何 Terminal 命令都必须在执行前展示不可变的操作预览，并由用户逐次确认。
...
- 首次操作时按需申请，不在首次启动时索要大权限。
- `workspace.read` 可选择“仅本次”或“本次会话”；持久授权必须由用户主动选择。
- `workspace.write` 和 `terminal.execute` 的基础授权最多持续到本次会话结束；具体写操作和命令仍需逐次确认。
- 高风险操作绝不提供“始终允许”。
...
- 任何 Terminal 命令进入 R3 确认卡；执行内容与预览摘要严格一致。
...
- 拒绝、关闭弹窗或批准超时后不执行。
- session grant 重启后失效；persistent grant 仅覆盖明确允许的低风险能力。
...
当前基线：lint 通过、typecheck 通过、8/8 测试通过、生产构建通过。
...
4. 所有写入、移动、删除和 Terminal 命令都在执行前显示精确预览并“允许一次”。
...
2. V1 只操作用户通过系统目录选择器授权的目录。
3. 所有 Terminal 命令和文件变更都逐次确认；R4 操作直接阻止，不提供强行继续。
4. 发行路线按 Developer ID 直发 + Hardened Runtime + 公证设计，而不是以 Mac App Store 为首要目标。

方案确认后，再生成一份可直接交给 AI Coding Agent 的 Goal 提示词。提示词将明确包含：
