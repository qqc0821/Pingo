...
- **Keep work bounded** — actions are checked against workspace boundaries, risk rules, and the macOS Seatbelt sandbox.
- **Close the loop** — follow task progress, inspect command output, and see the final result.
...
Pingo combines workspace-scoped file tools, an intent-whitelisted terminal, macOS Seatbelt sandboxing, redacted audit logs, and recoverable destructive changes. The current desktop entry automatically confirms planned operations after these checks; the approval flow remains available to controlled runtime paths and tests.

## Architecture

- Electron + React + TypeScript (electron-vite), SCSS for styling.
- Renderer stays `contextIsolation: true`, `nodeIntegration: false`, `sandbox: true`; it receives task state and operation results through the preload whitelist.
...
Sends your raw question to the real model and lets it decide whether to call a read-only terminal intent; the test runner automates capability and command approvals. The authorized directory defaults to the current working directory:
